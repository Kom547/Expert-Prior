        ...
        while n_steps < n_rollout_steps:
            ...
            if adv_action_mask or args.unlimited_attack:
                if args.attack_method == 'fgsm':
                    ...
                    adv_state = PGD(clipped_adv_actions[:, 1], self.trained_agent, obs_tensor[:, :-2], device=self.device)
...
class AdversarialPPO(OnPolicyAdversarialAlgorithm, PPO):
    def __init__(self, algo, path, aargs,env_name, best_model_path,  *args, **kwargs):
        ...
        if aargs.expert_attack:
            ...
            self.expert_models = load_ensemble_models(expert_model_path, (28,), 2, device)
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if self.aargs.expert_attack:
                    ...
                    expert_dist = th.distributions. normal. Normal(expert_means, torch.square(expert_vars))
                ...
                #print(values.shape)
                if self.aargs.expert_attack and self.aargs.expert_prior == 'ValuePenalty':
                    value_loss = F.mse_loss(rollout_data.returns, values_pred - self.alpha * exp_kl)
                else:
...
if args.expert_attack:
    expert_msg = f'{args.expert_prior}_{args.expert_train_samples}'
else:
