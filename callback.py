...
from evaluation_v2 import evaluate_policy
import numpy as np
import os
...
class CustomEvalCallback(EvalCallback):
    def __init__(
        ...
        # 初始化CSV文件，写入表头
        if not os.path.exists(self.best_model_save_path+"/eval_log.csv"):
            with open(self.best_model_save_path+"/eval_log.csv", 'w', newline='') as f:
                writer = csv.writer(f)
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            # 保存到CSV
            with open(self.best_model_save_path+"/eval_log.csv", 'a', newline='') as f:
                writer = csv.writer(f)
            ...
            if mean_reward > self.best_mean_reward:
                ...
                if self.best_model_save_path is not None:
                    self.model.save(os.path.join(self.best_model_save_path, "best_model"), exclude=["optimizer","replay_buffer"])
