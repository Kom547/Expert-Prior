...
from expert_imitation_learning import Actor as Actor1,load_ensemble_models as load_ensemble_models1,sample_from_mixture as sample_from_mixture1
...
class AdversarialEGPPO(OnPolicyAdversarialAlgorithm_EG, PPO):
    def __init__(self, custom_args, best_model_path, *args, **kwargs):
        ...
        if custom_args.expert_attack:
            ...
            self.use_hypo = custom_args.use_hypo
            import math
            self.total_iterations = math.ceil(custom_args.train_step / custom_args.num_envs)
            # 专家模型目录
            ...
            if "ensemblev2_2" not in expert_model_path:
                self.expert_models = load_ensemble_models(expert_model_path, (28,), 2,self.expert_cnt, self.device)
            else:
                print('ens')
                self.expert_models = load_ensemble_models1(expert_model_path, (28,), 2, self.device)
    ...
    def train(self) -> None:
        ...
        if self.use_hypo:
            self.beta = max(1 * (1 - self.iteration / self.total_iterations), 0.01)
