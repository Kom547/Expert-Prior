...
class OnPolicyAdversarialAlgorithm(OnPolicyAlgorithm):
    ...
    def collect_rollouts(self, env, callback, rollout_buffer, n_rollout_steps):
        ...
        while n_steps < n_rollout_steps:
            ...
            # if adv_action_mask or self.aargs.unlimited_attack:
            #     if self.aargs.attack_method == 'fgsm':
            #         adv_state = FGSM_v2(clipped_adv_actions[:, 1], victim_agent=self.trained_agent,
            #                                 last_state=obs_tensor[:, :-2], device=self.device,epsilon=self.aargs.attack_eps)
            #         # adv_state = fgsm_attack(clipped_adv_actions[:, 1], victim_agent=self.trained_agent,
            #         #                          last_state=obs_tensor[:, :-2])
            #     elif self.aargs.attack_method == 'fabfgsm':
            #         adv_state = FAB_FGSM_v2(self.trained_agent, clipped_adv_actions[:, 1], obs_tensor[:, :-2], device=self.device)
            #     elif self.aargs.attack_method == 'pgd':
            #         adv_state = PGD(clipped_adv_actions[:, 1], self.trained_agent, obs_tensor[:, :-2], device=self.device,epsilon=self.aargs.attack_eps)
            #     elif self.aargs.attack_method == 'cw':
            #         adv_state = cw_attack_v2(self.trained_agent, obs_tensor[:, :-2], clipped_adv_actions[:, 1])
            #
            #     if self.aargs.attack_method == 'direct':
            ...
            #print('generate adv cost:',time.time() - aastart)
        ...
        if adv_action_mask.any():
            ...
            if self.aargs.attack_method == 'fgsm':
                adv_state = FGSM_v2(selected_adv_actions, victim_agent=self.trained_agent,
                                    last_state=selected_states, epsilon=self.aargs.attack_eps, device=self.device)
            elif self.aargs.attack_method == 'pgd':
                adv_state = PGD(selected_adv_actions, self.trained_agent, selected_states, epsilon=self.aargs.attack_eps,
                                device=self.device)
            elif self.aargs.attack_method == 'cw':
                adv_state = cw_attack_v2(self.trained_agent, selected_states, selected_adv_actions,
                                         epsilon=self.aargs.attack_eps)
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            ...
            print("collect_rollouts time:", time.time() - cstart)
...
class AdversarialPPO(OnPolicyAdversarialAlgorithm, PPO):
    def __init__(self, algo, path, aargs,env_name, best_model_path,  *args, **kwargs):
        ...
        if aargs.best_model:
            model_path = os.path.join(path, env_name, algo, 'best_model/best_model')
        else:
        ...
        self.decouple = aargs.decouple
        self.unlimited_attack = aargs.unlimited_attack
        #for expert prior
        self.aargs = aargs
        ...
        if aargs.expert_attack:
            ...
            if aargs.expert_model_path == '':
                ...
                expert_model_path = aargs.expert_model_path
            self.expert_models = load_ensemble_models(expert_model_path, (28,), 2, self.device)
    ...
    def train(self) -> None:
        ...
        if self.iteration % 5 == 0:
            ...
            self.last_stage_eprewards  = self.last_stage_eprewards + [ep_info["r"] for ep_info in self.ep_info_buffer]
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if self.aargs.expert_attack:
                    expert_actions, expert_means, expert_vars = sample_from_mixture(obs, self.expert_models,
                                                             device=self.device)
...
def main():
    ...
    if args.expert_attack:
        #expert_msg = f'alt_ens2_{args.env_name}_{args.algo}_{args.expert_prior}_eps{args.attack_eps}'
