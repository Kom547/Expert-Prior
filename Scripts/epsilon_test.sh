  ...
  for eprior in ValuePenalty PolicyConstrained; do
    ...
    #algo=SAC
      ...
      #eps=0.09
        ...
        addition_msg="MoE_ensv2_beta_ens2_${env}_${algo}_${eprior}_eps${eps}_${seed}"
        ...
        cmd="python advTrain.py --cuda_seed ${seed} --n_steps 128 --train_step 200 --addition_msg ${addition_msg} --num_envs 8 --attack --env_name ${env} --algo ${algo} --attack_eps ${eps} --expert_model_path /data/lxy/STA-Expert/expert_model/MoE_ensv2_noES --expert_attack --cuda_number ${cuda_number} --expert_prior ${eprior}"
...
echo "启动 GPU0 上的任务..."
( cat "$gpu0_cmd" | xargs -I {} -P 15 bash -c '{}' ) &

echo "启动 GPU1 上的任务..."
( cat "$gpu1_cmd" | xargs -I {} -P 15 bash -c '{}' ) &
