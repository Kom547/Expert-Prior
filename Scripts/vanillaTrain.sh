...
#  for eprior in ValuePenalty PolicyConstrained; do
    #for algo in PPO SAC; do
      ...
      for eps in $(seq 0.05 0.01 0.10); do
        ...
        cmd="python advTrain.py --cuda_seed ${seed} --train_step 200 --addition_msg ${addition_msg} --num_envs 8 --attack --env_name ${env} --algo ${algo} --attack_eps ${eps} --cuda_number ${cuda_number}"
