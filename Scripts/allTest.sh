...
#        cmd="python evaluation_v3.py --result_saving --result_filename AttackEvaluation_TD3fix --algo ${algo} --env_name ${env} --expert_prior ${eprior} --train_step 100 --attack_eps ${eps} --cuda_number ${cuda_number} --attack --best_model --addition_msg ${addition_msg}"
...
#  for eprior in ValuePenalty PolicyConstrained; do
    #for algo in PPO SAC TD3; do
      ...
      for eps in $(seq 0.05 0.01 0.1); do
        ...
        #cuda_number=0
        # 拼接完整的命令，根据原命令模板
        cmd="python evaluation_v3.py --result_saving --result_filename AttackEvaluation_TD3fix --algo ${algo} --env_name ${env} --train_step 100 --attack_eps ${eps} --attack --cuda_number ${cuda_number} --best_model --addition_msg ${addition_msg}"
        # 分发到对应 GPU 的命令文件中
        log_file="logs/test_log/${addition_msg}.log"
        if [ $cuda_number -eq 0 ]; then
            echo "$cmd > $log_file 2>&1 && echo 'Job finished: $addition_msg' || echo 'Job failed: $addition_msg'" >> "$gpu0_cmd"
