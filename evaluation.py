...
if args.expert_recording:
    os.makedirs('./expert_data/{}_{}_{}'.format(args.env_name,args.algo,args.adv_algo), exist_ok=True)  # 创建数据保存目录
    shutil.rmtree('./expert_data/{}_{}_{}'.format(args.env_name,args.algo,args.adv_algo))  # 清理旧数据
    os.makedirs('./expert_data/{}_{}_{}'.format(args.env_name,args.algo,args.adv_algo), exist_ok=True)

#专家模型目录
if args.expert_attack:
    if args.expbase_algo:
        expbase_algo = args.expbase_algo
    else:
        expbase_algo = args.algo
    expert_model_path = f'./expert_model/{args.env_name}_{expbase_algo}_{args.adv_algo}_{args.expert_train_samples}'
...
args.eval = True
# 创建环境
env = gym.make(args.env_name, attack=args.attack, adv_steps=args.adv_steps, eval=args.eval)
env.unwrapped.start()
...
if args.attack:
    if args.best_model:
        ...
        # advmodel_path = "./logs/adv_eval/" + os.path.join(args.adv_algo, args.env_name, args.algo, args.addition_msg,
        #                                                   'lunar')
        advmodel_path = "./logs/adv_eval/" + os.path.join(args.adv_algo, args.env_name, expbase_algo, args.addition_msg,
...
for episode in range(args.train_step):
    ...
    for steps in range(args.T_horizon):
        ...
        if args.attack:
            ...
            #专家攻击
            if args.expert_attack:
                expert_actions,_,_ = sample_from_mixture(obs, expert_models)
            ...
            if adv_flag:
                print('steps ', episode_steps, 'pre actions', actions, 'adv_actions', adv_actions)
            ...
            if adv_action_mask or args.unlimited_attack:
                if args.attack_method == 'fgsm':
                    adv_state = FGSM_v2(adv_actions[1], victim_agent=trained_agent, last_state=obs_tensor[:-2],
                                        device=device,epsilon=args.attack_eps)
    ...
    # 在episode结束后保存数据
    if args.expert_recording:
        save_path = './expert_data/{}_{}_{}/episode_{}.npz'.format(args.env_name,args.algo,args.adv_algo,episode)
        if args.attack:
            np.savez(
                save_path,
